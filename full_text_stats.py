...
all_topic = []
for topic in os.listdir('news'):
    if len(list(os.listdir('news/' + topic))) > 1:
...
all_story = []
for topic in os.listdir('news'):
    for story in os.listdir(f'news/{topic}')[1:]:  # get rid of the 0-logs.json file
        all_story.append(f'news/{topic}/{story}')
